...
;
(function() {
  var getParameter, getPermalinkQuery, getQueryString, getResults, getTemplateOutput, isEmbed, isFormComplete, isNewQuery, isPermalinkPage, printResult, pushState, queryAPI, renderError, renderResults, roundNumber, syncFormWithPermalink;
  ...
  pushState = function(query) {
    ...
    if ($.support.history) {
      return history.pushState(query, '', url);
    }
  };
  ...
  isNewQuery = function(queryString) {
    return queryString !== window.location.search;
  };

  getQueryString = function(query) {
    return $.param(getPermalinkQuery(query));
  };
  ...
  getResults = function(query, options) {
    ...
    if (!isEmbed() && (isNewQuery(newQuery) || !history.state)) {
      pushState(query);
    }
